...
var index_default = {
  async fetch(request, env) {
    ...
    // =========================
    // Queena 新版 Imagemap 圖片
    // =========================
    if (request.method === "GET") {
      const url = new URL(request.url);

      if (
        url.pathname === "/menu-v2/1040" ||
        url.pathname === "/menu-v3/1040" ||
        url.pathname === "/menu-v6/1040"
      ) {
      ...
      if (
        url.pathname === "/menu-v2/700" ||
        url.pathname === "/menu-v3/700" ||
        url.pathname === "/menu-v6/700"
      ) {
        const img = await fetch(
...
\u76F4\u63A5\u8F38\u5165\u5546\u54C1\u540D\u7A31\u5373\u53EF\u67E5\u8A62`
          ...
          try {
            ...
            const sheetProduct = await findProductFromSheet(
              text,
              SHEET_ID2,
              SHEET_NAME
            );
                ...
                messages: [
                  {
                    type: "image",
                    originalContentUrl:
                      "https://drive.google.com/file/d/1TI9cimJXKO_ofP_i1tHWvaJ-JJ-QCXlO/view",
                    previewImageUrl:
                      "https://github.com/queena588588-create/line-carousel-bot/blob/main/%E7%BE%A4%E7%B5%84%E6%AD%A1%E8%BF%8E%E5%9C%96.png?raw=true"
                  },
                  {
                    type: "text",
...
\u8F38\u5165\u300C\u79C1\u8A0A\u300D\u4E00\u5C0D\u4E00\u79C1\u8A0A\u804A\u804A`
        ...
        if (
          ...
          const dedupeKey =
            `${sourceId}:${userId}:${text}`;

          const nowTime = Date.now();

          if (recentMessages.has(dedupeKey)) {
            const lastTime =
              recentMessages.get(dedupeKey);
...
// 私訊 Queena
// =========================
async function replyPrivateButton(
  replyToken,
  token
) {
  const message = {
...
// 購物車輪播
// =========================
async function replyCarouselFromSheet(
  replyToken,
  token
) {
  const url =
...
// 小幫手
// =========================
async function replyHelperButtons(
  replyToken,
  token
) {
  await fetch(
...
// 商品搜尋
// =========================
async function findProductFromSheet(
  keyword,
  SHEET_ID2,
  SHEET_NAME
) {
  const url =
    `https://docs.google.com/spreadsheets/d/${SHEET_ID2}/gviz/tq?tqx=out:csv&sheet=${encodeURIComponent(
      SHEET_NAME
    )}`;
...
\u53EA\u5269\u{1F6A8} ${parts.join("")}\uFF0C\u{1F6A8}\u5373\u5C07\u7D50\u55AE\uFF0C\u903E\u671F\u4E0D\u5019\u203C\uFE0F${
      closingText
        ? "\n\n" +
          closingText
        : ""
...
// 商品資訊
// =========================
async function replySheetProduct(
  replyToken,
  token,
  product
) {
  const text =
    `\u{1F6D2} ${product.productName}
...
async function findNoticeFromSheet(
  command
) {
  const rows =
    await readSheetCsv(
...
// 影片按鈕清單
// =========================
async function replyVideoButtons(
  replyToken,
  token
) {
  const url =
        ...
        contents: [
          {
            ...
            url:
              item.imageUrl ||
              "https://dummyimage.com/120x120/f5f5f5/999999.png&text=VIDEO",
            size: "sm",
            aspectRatio:
              "1:1",
            aspectMode:
...
// 最近結單
// =========================
async function replyClosingFlexList(
  replyToken,
  token
) {
  const url =
      ...
      contents: [
        {
          ...
          url:
            item.imageUrl ||
            "https://dummyimage.com/120x120/f5f5f5/999999.png&text=SALE",
          size:
            "sm",
          aspectRatio:
            "1:1",
                ...
                body: {
                  ...
                  contents: [
...

                    {
                      ...
                      text:
                        "\u{1F451} Queena \u79C1\u5FC3\u63A8\u85A6 \u{1F4A5}\u903E\u671F\u4E0D\u5019",
...
// 快速按鈕
// =========================
async function replyQuickButtons(
  replyToken,
  token,
  text,
  buttons
...
// 分類首頁
// =========================
async function replyProductCategoryHome(
  replyToken,
  token,
  sheetId,
  sheetName
...
// 分類商品列表
// =========================
async function replyProductCategoryList(
  replyToken,
  token,
  sheetId,
  sheetName,
...
async function replyUvOnly(
  ...
  function uvLevel(n) {
    ...
    if (
      ...
      return "\u4E2D\u91CF\u7D1A \u{1F4A7}\u66EC\u9ED1";
    }

    if (
      n <= 7
    ) {
      return "\u9AD8\u91CF\u7D1A \u{1F321}\uFE0F\u66EC\u50B7";
    }
  ...
  function uvAdvice(n) {
    ...
    if (
      ...
      return "\u{1F495}\uFF1A\u{1F324}\uFE0F \u7D2B\u5916\u7DDA\u8F03\u4F4E\uFF0C\u9752\u6625\u9632\u66EC\u8A18\u5F97\u64E6";
    }

    if (
      n <= 5
    ) {
      return "\u{1F495}\uFF1A\u{1F9F4} \u9752\u6625\u9632\u66EC\u8A18\u5F97\u64E6\uFF0C\u9069\u6642\u88DC\u6C34\u6216\u8292\u679C\u8336";
    }
...
// 即時天氣
// =========================
async function replyMorningWeather(
  replyToken,
  token,
  env
) {
...
// 天氣摘要
// =========================
async function replyWeatherSummary(
  replyToken,
  token,
  env
) {
...
// =========================
// Queena 新版首頁 Imagemap
// =========================
async function replyCuteHome(
  replyToken,
  token
) {
  const message = {
    ...
    baseUrl:
      "https://line-carousel-bot.queena-588-588.workers.dev/menu-v6",
